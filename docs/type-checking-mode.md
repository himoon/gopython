...
- 하지만 책의 예제 코드는 학습 목적상 간결하게 작성되어 있으며, 모든 코드에 타입 힌트가 붙어 있지는 않습니다.
- 또한 pandas, streamlit, 크롤링/자동화 관련 라이브러리처럼 동적으로 동작하는 도구들은 실제 실행에는 문제가 없어도 이 기능에서는 오류처럼 보일 수 있습니다.

### 🚨 핵심 원인
- **실행 오류와 편집기 경고는 다름**: 코드는 정상 실행되지만 VS Code가 코드를 미리 점검하는 단계에서 타입을 추론하지 못할 수 있음
- **확장 프로그램 설정 강화**: 업데이트 또는 설정 변경으로 Type Checking Mode가 `basic` 또는 `strict`로 활성화될 수 있음
- **학습용 예제의 특성**: 설명과 실습 중심 코드에서는 엄격한 타입 힌트를 모두 작성하지 않는 경우가 많음
