...
## 📋 목차
1. [실행 중인 터미널 창 및 대화형 창 종료](#1-실행-중인-터미널-창-및-대화형-창-종료)
2. [터미널 기본 프로필 변경 (윈도우 사용자)](#2-터미널-기본-프로필-변경-윈도우-사용자)
3. [파이썬 가상환경 설치](#3-파이썬-가상환경-설치)
4. [VS Code 새로고침](#4-vs-code-새로고침)
5. [파이썬 가상환경 활성화 확인](#5-파이썬-가상환경-활성화-확인)
6. [추가 도움이 필요하다면](#6-추가-도움이-필요하다면)
...
- 만약 `가상 환경 이름 입력` 창이 나타나는 경우 기본값인 `.venv`를 그대로 사용하려면 `Enter` 키를 누르세요. 다른 이름을 사용하려면 원하는 이름을 입력한 후 `Enter` 키를 누릅니다.
  ...
  <img src="https://raw.githubusercontent.com/himoon/gopython/refs/heads/main/images/venv-05-5.png" alt="가상 환경 이름 입력" width="700"/>
...
가상환경 설정 중 오류가 발생하거나 도움이 필요하신 경우, 오픈 채팅으로 문의해 주세요.
