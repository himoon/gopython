...
> 📢 **네이버 지도 UI가 업데이트되면서 일부 실습 코드가 정상적으로 동작하지 않을 수 있습니다.**
> 변경된 UI에 맞춰 관련 코드를 수정하였으니, 실습 전에 아래 내용을 확인해 주세요! 😊

### [step_1_3.py](step_1_3.py)
- `slow_mo=1000` → `slow_mo=2000`: 브라우저 동작 속도를 늦춰 안정성 향상
- 검색창 클릭 방식 변경: `get_by_label("장소, 버스, 지하철, 도로 검색").click()` → `get_by_role("button", name="검색").click()`
- 키워드 입력 방식 변경: `get_by_label("장소, 버스, 지하철, 도로 검색").fill()` → `get_by_role("combobox", name="장소, 버스, 지하철, 주소 검색").fill()`
  - 네이버 지도 UI 변경에 따라 locator를 `get_by_label` → `get_by_role` 방식으로 교체
